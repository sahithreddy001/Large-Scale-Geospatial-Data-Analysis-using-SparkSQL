...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = 
  {
    ...
    val my_hotzoneDF = spark.sql("select joinResult.rectangle, COUNT(joinResult.point) as count from joinResult GROUP BY joinResult.rectangle ORDER BY joinResult.rectangle")
    my_hotzoneDF.show()

    return my_hotzoneDF.coalesce(1)
  }
}
